Group: Private NSE equity trading group
Analyst: Sam
...
Signal delivery
Sorted ascending by price so members can scan from lowest to highest entry cost.
GTT values must always be provided so members can set and forget on their broker.
...
Output format
All signal blocks must use the structured table layout defined in AGENTS.md.
Every scan begins with a session header line identifying the time and session name.
No plain-text block format. Tables only.
